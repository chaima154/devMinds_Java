...
public class AdminIndexTrancheController implements Initializable {
    ...
    public AdminIndexTrancheController() throws SQLException {
    }
        ...
        trancheSearchBar.textProperty().addListener((Observable, oldValue, newValue) -> {
            if (newValue == null || newValue.isEmpty()) {
                ...
                filter.setPredicate(predicateTranche ->
                        String.valueOf(predicateTranche.getId()).contains(searchKey) ||
                                String.valueOf(predicateTranche.getMontantPaiement()).contains(searchKey) ||
                                predicateTranche.getDateEcheance().toString().contains(searchKey) ||
                                predicateTranche.getStatutPaiement().toLowerCase().contains(searchKey)
                );
            }
            trancheTableView.requestFocus(); // Request focus to the table view after filtering
        });
